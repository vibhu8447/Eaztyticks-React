window.addEventListener("message", function (event) {
  if (
    ...
    if (event.data.cmd == "findUserId") {
      ...
      if (
        window.Store.Status._index[Object.keys(window.Store.Status._index)[0]]
          .__x_id.user
      ) {
      ...
      for (var i = 0; i < b.length; i++) {
        x = {
          ...
          id: {
            server: "g.us",
            user: b[i].__x_id.user,
            _serialized: b[i].__x_id._serialized,
          },
        };
      ...
      chatid = event.data.id;
      imgBase64 = event.data.ImgData;
      // console.log(chatid);

      filename = "";
      caption = event.data.message;
      // caption=" "
      done = undefined;
      imgBase64 = "data:image/png;base64," + imgBase64;
      console.log(chatid, imgBase64, filename, caption);
      window.WAPI.sendImage(imgBase64, chatid, filename, caption, done);
    }
  }
});
